...
<body>
<a href="?" style="font-size: 18px;" align="center">Contact us(jshanggu@stevens.edu)</a><hr/>
HTML;
...
while($count && $rows=$rs->fetch_assoc()){
    ...
    if($cookie){
		$admin ="<a href=\"?reply&id={$id}\">Manage</a>  |  ";
	}else{
...
<form method="get"><hr />
    ...
    />
    page
    <input type="submit" value="Submit"/> ã€€|ã€€
    Every page[{$size}ï¼½
    Total[{$count}ï¼½ |
